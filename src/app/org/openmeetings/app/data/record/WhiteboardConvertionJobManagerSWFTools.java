...
public class WhiteboardConvertionJobManagerSWFTools {
	...
	public synchronized void initJobs() {
		try {
			if (!isRunning) {
				...
				List<Recording> recordingsNonConversionStarted = recordingDao
						.getRecordingWhiteboardToConvert();
				...
				for (Recording recording : recordingsNonConversionStarted) {
					...
					recordingConversionJobDao
							.addRecordingConversionJob(recordingConversionJob);

				}

				// Do SVG Conversion for next 100 SVG Images, that should fill
				// one Folder
	...
	public synchronized void processJobs() {
		try {
			...
			List<RecordingConversionJob> listOfConversionJobs = recordingConversionJobDao
					.getRecordingConversionJobs();
			...
			for (RecordingConversionJob recordingConversionJob : listOfConversionJobs) {
				...
				if (recordingConversionJob.getEndTimeInMilliSeconds()
						.equals(0L)) {
					...
					String roomRecordingInXML = recordingConversionJob
							.getRecording().getRoomRecording()
							.getInitwhiteboardvarsInXml();

					Map initWhiteBoardObjects = (Map) xStream_temp
							.fromXML(roomRecordingInXML);

					this.generateFileAsSVG(initWhiteBoardObjects,
							roomRecordingInXML, recordingConversionJob);

				} else {

					log.debug("DRAW NEXT IMAGE");

					if (recordingConversionJob.getRecording().getDuration() >= recordingConversionJob
						...
						List<WhiteBoardEvent> whiteBoardEventList = whiteBoardEventDao
								.getWhiteboardEventsInRange(
										...
										recordingConversionJob.getRecording()
												.getRoomRecording()
												.getRoomrecordingId());

						log.debug("whiteBoardEventList SIZE "
								+ whiteBoardEventList.size());

						XStream xStream_temp = new XStream(new XppDriver());
						xStream_temp.setMode(XStream.NO_REFERENCES);

						String roomRecordingInXML = recordingConversionJob
								.getCurrentWhiteBoardAsXml();
						Map whiteBoardObjects = (Map) xStream_temp
								.fromXML(roomRecordingInXML);
						...
						for (WhiteBoardEvent whiteBoardEvent : whiteBoardEventList) {
							...
							log.debug("whiteBoardEvent: "
									+ whiteBoardEvent.getStarttime());

							XStream xStream_temp_action = new XStream(
									new XppDriver());
							xStream_temp_action.setMode(XStream.NO_REFERENCES);

							Map actionObj = (Map) xStream_temp_action
									.fromXML(whiteBoardEvent.getAction());
							...
							if (action.equals("draw") || action.equals("redo")) {
								...
								String objectOID = actionObject.get(
								...
								String objectOID = actionObject.get(
										actionObject.size() - 1).toString();
								// Map roomItem = (Map)
								// whiteBoardObjects.get(objectOID);
								whiteBoardObjects.put(objectOID, actionObject);
						...
						XStream xStream_temp_store = new XStream(
								new XppDriver());
						xStream_temp_store.setMode(XStream.NO_REFERENCES);
						String roomRecordingInXMLToSave = xStream_temp_store
								.toXML(whiteBoardObjects);

						this.generateFileAsSVG(whiteBoardObjects,
								roomRecordingInXMLToSave,
								recordingConversionJob);

					} else {

						log.debug("THIS FILE IS PROCESSED UPDATE: "
								+ recordingConversionJob
										.getRecordingConversionJobId());

						// FIXME: this should happen only one time per
						// conversion Job

						recordingConversionJob.setEnded(new Date());
						recordingConversionJob
								.setStartedPngConverted(new Date());
						recordingConversionJob.setBatchProcessCounter(0L);
						recordingConversionJobDao
								.updateRecordingConversionJobs(recordingConversionJob);
	...
	public synchronized void processConvertionJobs() {
		try {
			...
			for (RecordingConversionJob recordingConversionJob : listOfConversionJobs) {
				...
				Double maxFolderDoub = Math.floor(recordingConversionJob
						.getImageNumber() / maxNumberOfBatchFolderSize);

				int maxFolder = maxFolderDoub.intValue();

				if (maxFolder >= recordingConversionJob
						.getBatchProcessCounter()) {

					Map<String, String> outputFileNames = null;
					if (isDebug) {
						outputFileNames = this
								.generateBatchFileDebug(recordingConversionJob
										.getRecordingConversionJobId(),
										recordingConversionJob
												.getBatchProcessCounter());
					} else {
						outputFileNames = this
								.generateBatchFile(recordingConversionJob
										.getRecordingConversionJobId(),
										recordingConversionJob
												.getBatchProcessCounter());
					}
					...
					generateImage.convertImageByTypeAndSizeAndDepth(
							...
							outputFileNames.get("output"), 660, 580, depth);

					// Add Count For next Round
					RecordingConversionJob recordingConversionJobUpdate = recordingConversionJobDao
							.getRecordingConversionJobsByRecordingConversionJobsId(recordingConversionJob
									.getRecordingConversionJobId());

					recordingConversionJobUpdate
							.setBatchProcessCounter(recordingConversionJob
									.getBatchProcessCounter() + 1);
					recordingConversionJobDao
							.updateRecordingConversionJobs(recordingConversionJobUpdate);
					...
					recordingConversionJobUpdate
							.setStartedSWFConverted(new Date());
					recordingConversionJobDao
							.updateRecordingConversionJobs(recordingConversionJobUpdate);
	...
	public synchronized void processConvertionSWFJobs() {
		try {
			...
			List<RecordingConversionJob> listOfSWFConversionJobs = recordingConversionJobDao
			...
			for (RecordingConversionJob recordingConversionJob : listOfSWFConversionJobs) {
				...
				for (long i = 0; i < maxFolder; i++) {
					...
					String folderName = this.getBatchFileFolder(
							recordingConversionJob
									.getRecordingConversionJobId(), i);

					for (int k = 0; k < maxNumberOfBatchFolderSize; k++) {

						// three times to simulate a FPS of 30 while we only
						// generate
				...
				long restImages = recordingConversionJob.getImageNumber()
				...
				for (int k = 0; k < restImages; k++) {
					...
					String folderName = this.getBatchFileFolder(
							recordingConversionJob
									.getRecordingConversionJobId(), maxFolder);
				...
				String output = this.getSWFFileForResult(recordingConversionJob
						.getRecordingConversionJobId());

				String outputFolder = this
						.getSWFFolderForResult(recordingConversionJob
								.getRecordingConversionJobId());

				// Generate that SWF only by 1000 Files as png2swf.c has a
				// MAX_INPUT_FILE of 1024

				// This workaround is just to workaround the 1024 File LIMIT of
				// SWFTools!!!

				List<String> outputfiles = new LinkedList<String>();

				int maxnumber = Double
						.valueOf(Math.floor(images.size() / 1000)).intValue();
				...
				if (maxnumber > 0) {
					for (int i = 0; i <= maxnumber; i++) {
						...
						log.debug("Generate SWF for Items: " + i + " TO : "
								+ (i * 1000));
						String fileName = outputFolder + File.separatorChar
								+ "whiteboard" + i + ".swf";
						...
						for (int k = 0; k < 1000; k++) {
							int index = k + (i * 1000);
							log.debug("index Number " + index + " size: "
									+ images.size());
						...
						generateSWF.generateSwfByImages(imageForConvert,
								fileName, 30);
					}

					// Combine Resulting SWFs to one SWF

					// FIXME: IT SEEMS LIKE SWFCOMBINE DOES PRODUCE INVALID
					// Files!!
					generateSWF.generateSWFByCombine(outputfiles, output, 30);

				} else {

					// Directly write to SWF
					generateSWF.generateSwfByImages(images, output, 30);
	...
	private void generateFileAsSVG(Map whiteBoardObjects,
			String roomRecordingInXML,
			RecordingConversionJob recordingConversionJob) throws Exception {
		...
		RecordingConversionJob recordingConversionJobToStore = recordingConversionJobDao
				.getRecordingConversionJobsByRecordingConversionJobsId(recordingConversionJob
						.getRecordingConversionJobId());

		recordingConversionJobToStore
				.setEndTimeInMilliSeconds(recordingConversionJob
						.getEndTimeInMilliSeconds() + numberOfMilliseconds);
		recordingConversionJobToStore
				.setCurrentWhiteBoardAsXml(roomRecordingInXML);
		recordingConversionJobToStore.setImageNumber(recordingConversionJob
				.getImageNumber() + 1);

		log.debug("updateRecordingConversionJobs: generateFileAsSVG");
		recordingConversionJobDao
				.updateRecordingConversionJobs(recordingConversionJobToStore);

	}

	private String generateSVGFileDebug(Long conversionJobId, Long imageNumber)
			throws Exception {
	...
	/**
	 * This won't work in Debug(=>means JUnit!) Modus as the batch File-Dir is
	...
	private Map<String, String> generateBatchFile(Long conversionJobId,
		...
		String recordingRootDir = ScopeApplicationAdapter.webAppPath
	...
	private String getBatchFileFolder(Long conversionJobId, Long folderNumber)
			throws Exception {
	...
	private String getSWFFolderForResult(Long conversionJobId) throws Exception {
		...
		String recordingFileDir = recordingRootDir + File.separatorChar
				+ conversionJobId;
		String recordingFileDirAbsolute = (new File(recordingFileDir))
				.getAbsolutePath();
