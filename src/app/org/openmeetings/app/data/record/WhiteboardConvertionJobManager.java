...
public class WhiteboardConvertionJobManager {
	...
	private static final Logger log = Red5LoggerFactory.getLogger(
			WhiteboardConvertionJobManager.class,
			...
			for (RecordingConversionJob recordingConversionJob : listOfConversionJobs) {
				...
				if (maxFolder >= recordingConversionJob
					...
					generateImage.convertImageByTypeAndSizeAndDepth(
							...
							outputFileNames.get("output"), defaultWidth, 580,
							depth);
			...
			for (RecordingConversionJob recordingConversionJob : listOfSWFConversionJobs) {
				...
				String outputFolder = this
						.getSWFFolderForResult(recordingConversionJob
								.getRecordingConversionJobId());
				String pngOutputFolder = outputFolder + File.separatorChar
						+ "PNG";
				...
				// Move Files to result folder and reflect that we need 30 FPS
				// instead of 5 FPS in the initial Break Down
				int i = 0;
				for (Iterator<String> iter = images.iterator(); iter.hasNext();) {
					...
					FileHelper.copy(new File(fileToMove), new File(
							pngOutputFolder + File.separatorChar + "" + i
									+ ".png"));
					i++;
					FileHelper.copy(new File(fileToMove), new File(
							pngOutputFolder + File.separatorChar + "" + i
				...
				String pngOutputFolderWildCard = pngOutputFolder
						+ File.separatorChar + "%d.png";

				generateSWF.generateSWFByFFMpeg(pngOutputFolderWildCard,
	...
	private void generateFileAsSVG(Map whiteBoardObjects,
		...
		log.debug("updateRecordingConversionJobs: generateFileAsSVG: "
				+ recordingConversionJobToStore.getEnded());
		recordingConversionJobDao
