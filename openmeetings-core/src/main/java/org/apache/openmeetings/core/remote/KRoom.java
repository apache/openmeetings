...
public class KRoom {
	...
	public KRoom(IKurentoHandler handler, Room r, IClientManager cm, RecordingDao recDao) {
	...
	public KStream join(final StreamDesc sd, IKurentoHandler kHandler) {
		...
		final KStream stream = new KStream(sd, this, kHandler, this.processor);
	...
	public void startSharing(IClientManager cm, Client c, Optional<StreamDesc> osd, JSONObject msg, Activity a) {
		StreamDesc sd;
		if (sharingStarted.compareAndSet(false, true)) {
			...
			WebSocketHelper.sendRoomOthers(room.getId(), c.getUid(), kHandler.newKurentoMsg()
					.put("id", "newStream")
					.put(PARAM_ICE, kHandler.getTurnServers(c))
					.put("stream", sd.toJson()));
		}
